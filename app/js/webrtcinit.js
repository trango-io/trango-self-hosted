...
function initWebSocket() {
  ...
  // let dataName = document.getElementById(pData.from).getAttribute('data-name');

  //ahref.appendChild(span);
  li.appendChild(span);

  // var myPeer = document.getElementById("myPeer");
  ...
  ws.onmessage = async function (evt) {
    ...
    if (message.type === 'offer') {
      ...
      var noPeerDiv = document.getElementById('nopeerdiv');
      if (document.getElementById("peers").getElementsByTagName("li").length <1 &&  noPeerDiv.classList.contains('hide-div')) {
        // It has at least one
        noPeerDiv.classList.remove("hide-div");
      }
...
function AddPeer(device) {
  ...
  var noPeerDiv = document.getElementById('nopeerdiv');
  if (!noPeerDiv.classList.contains('hide-div')) {
    noPeerDiv.classList.add("hide-div");
  }
  if (peer == null) {
    return;
  }
  ...
  if (device.hasOwnProperty("devtype") && device.hasOwnProperty("email")
    ...
    span.innerHTML = device.name; // + " " + device.devtype;
    li.appendChild(span);
